...
def dashboard():
    ...
    total_asset_count = Asset.query.count()
    total_license_count = License.query.filter(License.expiration_date > today).count()


    total_asset_in_repair_count = 0 # License.query.filter(License.expiration_date >= today).count()
    number_of_license_due_expiry_count = License.query.filter(License.expiration_date <= today).count()
